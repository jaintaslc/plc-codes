...
def hardware_init():
    ...
    GPIO.setup(GPIO_RANGE_TRIGGER, GPIO.OUT)
    GPIO.setup(GPIO_RANGE_ECHO, GPIO.IN)
...
def get_distance():
    # set Trigger to HIGH
    GPIO.output(GPIO_RANGE_TRIGGER, True)

    # set Trigger after 0.01ms to LOW
    time.sleep(0.00001)
    GPIO.output(GPIO_RANGE_TRIGGER, False)

    StartTime = time.time()
    StopTime = time.time()

    # save StartTime
    while GPIO.input(GPIO_RANGE_ECHO) == 0:
        StartTime = time.time()

    # save time of arrival
    while GPIO.input(GPIO_RANGE_ECHO) == 1:
        StopTime = time.time()
...
def update_inputs():
    ...
    distance = int(get_distance()*100)
    print("getting rgb...")
    color_rgb = rgbsensor.color_rgb_bytes
    print("Range sensor: ",str(distance/100.0))
    print("Color tuple: ",str(color_rgb))
    psm.set_var("IW0", color_rgb[1])
    psm.set_var("IW1", color_rgb[2])
    psm.set_var("IW2", color_rgb[0])
    psm.set_var("IW3", distance)
...
def update_outputs():
    ...
    print("pump: ",pump)
    print("doser_red: ",doser_red)
    print("doser_blue: ",doser_blue)
